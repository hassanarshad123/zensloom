...
fn test_swscale_conversion_works() {
    ...
    let config = zensloom_frame_converter::ConversionConfig::new(
    ...
    let result = zensloom_frame_converter::create_converter_with_details(config);
    assert!(
        result.is_ok(),
...
fn test_system_diagnostics_collection() {
    ...
    let diagnostics = zensloom_recording::diagnostics::collect_diagnostics();
...
fn test_gpu_detection() {
    ...
    let gpu_info = zensloom_frame_converter::detect_primary_gpu();
    ...
    if let Some(info) = gpu_info {
        ...
        match info.vendor {
            ...
            zensloom_frame_converter::GpuVendor::Intel => {
            ...
            zensloom_frame_converter::GpuVendor::Microsoft => {
                println!("  -> Microsoft WARP: Software rendering/encoding");
            }
            zensloom_frame_converter::GpuVendor::Unknown(id) => {
...
fn test_camera_enumeration() {
    ...
    let cameras: Vec<zensloom_camera::CameraInfo> = zensloom_camera::list_cameras().collect();
...
fn test_encoder_availability_matrix() {
    ...
    let gpu = zensloom_frame_converter::detect_primary_gpu();
    println!("\n=== Recommended Encoder Priority ===");
    match gpu.map(|g| g.vendor) {
        Some(zensloom_frame_converter::GpuVendor::Nvidia) => {
            println!("  NVIDIA detected: h264_nvenc -> h264_mf -> h264_qsv -> h264_amf -> libx264");
        }
        Some(zensloom_frame_converter::GpuVendor::Amd) => {
            println!("  AMD detected: h264_amf -> h264_mf -> h264_nvenc -> h264_qsv -> libx264");
        }
        Some(zensloom_frame_converter::GpuVendor::Intel) => {
...
fn test_d3d11_converter_capability() {
    ...
    for (name, input, output, width, height) in test_configs {
        let config =
            zensloom_frame_converter::ConversionConfig::new(input, width, height, output, width, height);

        match zensloom_frame_converter::create_converter_with_details(config) {
            Ok(result) => {
...
fn test_supported_pixel_formats() {
    ...
    for (format, name) in formats {
        let supported = zensloom_frame_converter::is_format_supported(format);
...
fn test_frame_conversion_performance() {
    ...
    let result = zensloom_frame_converter::create_converter_with_details(config.clone());
    if result.is_err() {
...
fn test_multi_gpu_detection() {
    ...
    if let Some(gpu) = zensloom_frame_converter::detect_primary_gpu() {
...
fn test_minimum_requirements_check() {
    ...
    println!("\nRecommended:");
    if zensloom_frame_converter::detect_primary_gpu().is_some() {
