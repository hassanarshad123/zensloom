...
use zensloom_desktop_lib::DynLoggingLayer;
...
fn main() {
    ...
    let info_file_appender = tracing_appender::rolling::daily(&logs_dir, "zensloom.log");
    let (info_file_writer, _info_logger_guard) = tracing_appender::non_blocking(info_file_appender);

    let errors_file_appender =
        tracing_appender::rolling::daily(&logs_dir, "zensloom-errors.log");
    ...
    tracing_subscriber::registry()
        .with(tracing_subscriber::filter::filter_fn(
            (|v| v.target().starts_with("zensloom_") || v.target().starts_with("cap_")) as fn(&tracing::Metadata) -> bool,
        ))
        .with(reload_layer)
        .with(level_filter)
        .with(
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(zensloom_desktop_lib::run(handle, logs_dir));
}
...
fn install_panic_hook(logs_dir: std::path::PathBuf) {
    ...
    std::panic::set_hook(Box::new(move |info| {
        ...
        eprintln!(
            "[zensloom panic] thread '{thread_name}' at {location}: {message}\nbacktrace:\n{backtrace}"
        );
        prev(info);
    }));
}
